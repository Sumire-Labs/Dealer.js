...
### Bug Fixes
- **チンチロ**: ラウンド解決中のDB操作エラーでセッションが永久に残り、チャンネルが「テーブル使用中」のまま新規ゲームを開始できなくなるバグを修正
- **チンチロ**: 最後の操作から10分以上経過したスタルセッションを自動クリーンアップするように変更

## v1.11.5

### Bug Fixes
- **ワーク**: シフト実行・残業・ジョブ選択などの全ボタン/セレクトメニューで `deferUpdate()` が欠落しており、DB処理中にDiscordの3秒タイムアウトが発生していた問題を修正。結果が表示されない・チップが反映されないケースを解消
- **ワーク**: クールダウンチェック後〜トランザクション完了前の間に連打すると複数リクエストが通過する競合状態を修正。楽観的ロック方式に変更し、失敗時はロールバック
