  ...
  try {
    ...
    for (const player of session.players) {
      try {
        ...
        const playResults = await updateMissionProgress(player.userId, { type: 'game_play', gameType: 'BLACKJACK' });
        allCompleted.push(...playResults);

        if (net > 0n) {
          const winResults = await updateMissionProgress(player.userId, { type: 'game_win', gameType: 'BLACKJACK' });
          allCompleted.push(...winResults);
          const earnResults = await updateMissionProgress(player.userId, { type: 'chips_earned', amount: Number(net) });
          allCompleted.push(...earnResults);
        }

        const betResults = await updateMissionProgress(player.userId, { type: 'chips_bet', amount: Number(result.totalBet) });
