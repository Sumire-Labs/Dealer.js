...
export function formatRequesterValue(user: RequesterFields, achievementCount: number, category: LeaderboardCategory): string {
    switch (category) {
        case 'chips':
            return formatChips(user.chips);
        case 'net_worth':
            return formatChips(user.chips + user.bankBalance);
        case 'total_won':
            return formatChips(user.totalWon);
        case 'work_level':
            return `Lv.${user.workLevel}`;
        case 'shop_spend':
            return formatChips(user.lifetimeShopSpend);
        case 'achievements':
            return `${achievementCount}個`;
    }
}
