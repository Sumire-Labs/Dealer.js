...
export async function borrowChips(userId: string, amount: bigint): Promise<{
    newBalance: bigint;
...
export async function repayChips(userId: string, amount: bigint): Promise<{
    newBalance: bigint;
    repaid: bigint;
    newlyUnlocked: AchievementDefinition[]
}> {
...
export async function declareBankruptcy(userId: string): Promise<{
    newBalance: bigint;
    newlyUnlocked: AchievementDefinition[]
}> {
    return prisma.$transaction(async (tx) => {
